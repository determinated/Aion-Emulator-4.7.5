...
@AIName("Anoha_sword_ely") // 804576 - 
public class AnohaSwordElyosAI2 extends NpcAI2 {	

    @Override
    protected void handleDialogStart(Player player) {
	

        if (player.getInventory().getItemCountByItemId(185000215) >= 1 && player.getRace() == Race.ELYOS) {		
    ...
    private void despawn() {
        ThreadPoolManager.getInstance().schedule(new Runnable() {
            ...
            public void run() {
                if (!isAlreadyDead()) {
                    AI2Actions.deleteOwner(AnohaSwordElyosAI2.this);
                }
            }
        }, 500);
    }
